...
- 처음 로그인한 외부 계정이면 `users`와 `oauth_identities`를 하나의 DB 트랜잭션으로 생성한다.
  ...
  DB 커밋 이후의 토큰 저장 실패는 [로그인 실패 처리](../login/LOGIN_FLOW.md#계정-생성-후-토큰-저장-실패)를 따른다.
...
인증 상태 저장은 [RT 설계](../token/REFRESH_TOKEN_DESIGN.md)와 [OAuth 임시 상태](../login/OAUTH_STATE.md)에서 관리한다.
