...
/**
 * BFF가 인증한 X-User-Id 헤더로 계정 프로필을 조회·수정한다.
 *
...
public class AccountController {
    ...
    @GetMapping
    public ResponseEntity<AuthResponses.Profile> get(HttpServletRequest request) {
        return profile(accounts.get(userId(request)));
    }

    @PatchMapping
    public ResponseEntity<AuthResponses.Profile> rename(
    ...
    private AuthFailure invalidRequest() {
        return new AuthFailure(AuthFailure.Reason.INVALID_REQUEST);
    }

    private ResponseEntity<AuthResponses.Profile> profile(AccountUseCase.Profile profile) {
        return ResponseEntity.ok()
                .header("Cache-Control", "no-store")
                .body(responses.profile(profile));
    }
}
