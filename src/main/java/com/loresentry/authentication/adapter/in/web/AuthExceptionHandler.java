...
public class AuthExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handle(Exception failure, HttpServletRequest request) {
        if (failure instanceof org.springframework.http.converter.HttpMessageNotReadableException
            ...
            return ErrorResponses.response(new AuthFailure(AuthFailure.Reason.INVALID_REQUEST,
                ErrorResponses.callback(request) ? AuthFailure.Consumption.NOT_CONSUMED : null), request);
        }
