...
public final class OAuthStateJson {
    ...
    public String encode(State value) {
        ...
        return mapper.writeValueAsString(
                Map.of(
                        ...
                        value.codeVerifier(),
                        "created_at",
                        value.createdAt().getEpochSecond(),
                        "expires_at",
                        value.expiresAt().getEpochSecond()));
    }
        ...
        try {
            ...
            for (var field :
                    Set.of(
                            ...
                            "state",
                            "nonce",
                            "code_verifier")) if (!n.get(field).isString()) throw invalid();
            for (var field : Set.of("schema_version", "created_at", "expires_at"))
                if (!n.get(field).isIntegralNumber() || !n.get(field).canConvertToLong())
                    throw invalid();
            if (n.get("schema_version").longValue() != 1) throw invalid();
            var value =
                    new State(
                            1,
                            n.get("registration_id").asString(),
                            n.get("client_id").asString(),
                            n.get("redirect_uri").asString(),
                            n.get("state").asString(),
                            n.get("nonce").asString(),
                            n.get("code_verifier").asString(),
                            Instant.ofEpochSecond(n.get("created_at").longValue()),
    ...
    private void validate(State v) {
        if (v == null
                || v.schemaVersion() != 1
                || !"google".equals(v.registrationId())
                || v.clientId() == null
                || v.clientId().isBlank()
                || v.redirectUri() == null
                || v.redirectUri().isBlank()
                || !OAuthSecrets.valid(v.state())
                || !OAuthSecrets.valid(v.nonce())
                || !OAuthSecrets.valid(v.codeVerifier())
                || v.createdAt() == null
                || v.expiresAt() == null
                || !v.createdAt().plusSeconds(300).equals(v.expiresAt())) throw invalid();
    }
