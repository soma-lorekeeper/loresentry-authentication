...
class RefreshStoreTest extends DatabaseTestSupport {
    ...
    @Test
    void storesOnlyUserWithRemainingTtlAndOneConcurrentConsumer() throws Exception {
        ...
        assertThatThrownBy(() -> store.save(jti, UUID.randomUUID(), Instant.now().plusSeconds(40)))
    ...
    @Test
    void rejectsExpiredAndMalformedStoredId() {
        var jti = UUID.randomUUID();
        assertThatThrownBy(() -> store.save(jti, UUID.randomUUID(), Instant.EPOCH))
        ...
        assertThatThrownBy(() -> store.consume(jti))
                .isInstanceOfSatisfying(
                        PortFailure.class,
    ...
    @Test
    void distinguishesConnectionAcquisitionFromLostCommandResponse() {
        ...
        assertThatThrownBy(() -> adapter.consume(UUID.randomUUID()))
                .isInstanceOfSatisfying(
                        PortFailure.class,
                        e -> {
                            assertThat(e.execution()).isEqualTo(PortFailure.Execution.NOT_EXECUTED);
                            assertThat(e.retryable()).isTrue();
                            assertThat(e).hasMessage("UNAVAILABLE").hasNoCause();
                        });
        var connection = mock(RedisConnection.class);
        var commands = mock(RedisStringCommands.class);
