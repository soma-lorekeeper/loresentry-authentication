...
class MigrationTest {
    ...
    void deployedEmptyV1UpgradesWithoutRewritingItsHistory() throws Exception {
        ...
        var v1 = Flyway.configure()
                .dataSource(postgres.getJdbcUrl(), postgres.getUsername(), postgres.getPassword())
                .schemas(schema)
                .target("1")
                .load();
        assertThat(v1.migrate().migrationsExecuted).isEqualTo(1);
        assertThat(tables(schema)).containsExactlyInAnyOrder("flyway_schema_history", "users", "auth_sessions");
        var checksum = v1.info().current().getChecksum();

        var flyway = migrations(schema);
        assertThat(flyway.migrate().migrationsExecuted).isEqualTo(1);
        assertThat(flyway.info().applied())
                .filteredOn(migration -> MigrationVersion.fromVersion("1").equals(migration.getVersion()))
    ...
    private void assertCurrentSchema(Flyway flyway, String schema) throws Exception {
        ...
        assertThat(flyway.info().applied())
                ...
                .containsExactly("1", "2");
        assertThat(tables(schema)).containsExactlyInAnyOrder("flyway_schema_history", "users", "oauth_identities");
        try (Connection connection = DriverManager.getConnection(
            ...
            try (var columns = connection.createStatement().executeQuery(
                ...
                assertThat(names).containsExactlyInAnyOrder("id", "display_name", "created_at", "updated_at");
            }
        }
    }
    ...
    private List<String> tables(String schema) throws Exception {
        try (Connection connection = DriverManager.getConnection(
                postgres.getJdbcUrl(), postgres.getUsername(), postgres.getPassword())) {
            connection.setSchema(schema);
            try (var tables = connection.createStatement().executeQuery(
