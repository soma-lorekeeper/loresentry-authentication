...
class FullLoginFlowTest extends HttpAuthTestSupport {
    ...
    @Test void productionCoreAndAdaptersAreWiredThroughConfiguration() {
        ...
        assertThat(context.getBean(RegisterIdentityUseCase.class)).isInstanceOf(RegistrationService.class);
        assertThat(accounts).isInstanceOf(JpaAccountStore.class); assertThat(states).isInstanceOf(RedisOAuthStateStore.class);
        assertThat(refreshStates).isInstanceOf(RedisRefreshTokenStore.class); assertThat(jwt).isInstanceOf(RsaJwtTokens.class);
        var provider = context.getBean(OidcClient.class); assertThat(provider).isInstanceOf(GoogleOidcClient.class);
        assertThat(ReflectionTestUtils.getField(context.getBean(LoginUseCase.class), "provider")).isSameAs(provider);
        assertThat(ReflectionTestUtils.getField(context.getBean(RefreshUseCase.class), "store")).isSameAs(refreshStates);
    }
    @Test void fullHttpLifecycleKeepsIdentityAndOtherDeviceState() throws Exception {
        var subject = "full-"+UUID.randomUUID(); var first = login(subject);
        ...
        assertThat(access.verify(new RSASSAVerifier((RSAPublicKey) TestKeys.PAIR.getPublic()))).isTrue();
        ...
        var profile = call("GET", "/auth/users/me", null, user); assertThat(profile.status()).isEqualTo(200);
        ...
        assertThat(renamed.status()).isEqualTo(200); assertThat(renamed.body().get("display_name").asString()).isEqualTo("My Name");
        var second = login(subject); assertThat(jwt.verifyRefresh(rt(second), false).userId()).isEqualTo(user);
        assertThat(call("GET", "/auth/users/me", null, user).body().get("display_name").asString()).isEqualTo("My Name");
        var oldJti = jwt.verifyRefresh(rt(first), false).jti(); var deviceJti = jwt.verifyRefresh(rt(second), false).jti();
        var rotated = refresh(rt(first)); assertThat(rotated.status()).isEqualTo(200);
        assertThat(refreshStates.consume(oldJti)).isEmpty();
        assertThat(rotated.body().has("login_request_consumed")).isFalse();
        var revoked = call("POST", "/auth/tokens/revoke", Map.of("refresh_token", rt(rotated)), null);
        assertThat(revoked.status()).isEqualTo(204); assertThat(revoked.body()).isNull();
