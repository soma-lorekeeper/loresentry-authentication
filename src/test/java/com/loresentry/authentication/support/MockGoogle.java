    ...
    public MockGoogle() {
        try {
            ...
            server.createContext("/jwks", exchange -> {
                ...
                var key = new RSAKey.Builder((RSAPublicKey) keys.getPublic()).keyID(kid).algorithm(JWSAlgorithm.RS256).build();
                respond(exchange, 200, new JWKSet(key).toString());
            });
            server.createContext("/", exchange -> { unexpectedCalls.incrementAndGet(); respond(exchange, 404, "{}"); }); server.start();
        } catch (Exception e) { throw new IllegalStateException(e); }
    }
    public String base() { return "http://127.0.0.1:" + server.getAddress().getPort(); }
    public GoogleOidcClient.Endpoints endpoints() { return new GoogleOidcClient.Endpoints(base()+"/authorize", base()+"/token", base()+"/jwks"); }
    public JWTClaimsSet.Builder claims(String nonce, String subject) {
        var now = Instant.now();
        return new JWTClaimsSet.Builder().issuer("https://accounts.google.com").audience("test-client").subject(subject)
            .issueTime(Date.from(now)).expirationTime(Date.from(now.plusSeconds(300))).claim("nonce", nonce).claim("name", "Test User").claim("email", "test@example.com");
    }
    public String sign(JWTClaimsSet claims) { return sign(claims, keys, kid, null); }
    public String sign(JWTClaimsSet claims, java.security.KeyPair pair, String keyId, URI headerUrl) {
        try {
            var header = new JWSHeader.Builder(JWSAlgorithm.RS256).keyID(keyId);
            if (headerUrl != null) header.jwkURL(headerUrl);
            var jwt = new SignedJWT(header.build(), claims); jwt.sign(new RSASSASigner((RSAPrivateKey) pair.getPrivate())); return jwt.serialize();
        } catch (Exception e) { throw new IllegalStateException(e); }
    }
    public static Map<String, String> form(String text) {
        Map<String, String> values = new HashMap<>();
        for (var part : text.split("&")) { var pair = part.split("=", 2); values.put(URLDecoder.decode(pair[0], StandardCharsets.UTF_8), pair.length == 2 ? URLDecoder.decode(pair[1], StandardCharsets.UTF_8) : ""); }
        return values;
    }
