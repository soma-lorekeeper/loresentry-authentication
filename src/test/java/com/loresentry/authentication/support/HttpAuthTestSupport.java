...
public abstract class HttpAuthTestSupport extends DatabaseTestSupport {
    ...
    public static class GoogleHttpConfiguration {
        @Bean(destroyMethod = "close") MockGoogle mockGoogle() { return new MockGoogle(); }
        @Bean(destroyMethod = "close") @Primary GoogleOidcClient controlledGoogleClient(GoogleSettings settings, Clock clock, MockGoogle google) {
    ...
    protected record Pending(String id, String state, String code) {
        public Map<String, String> body() { return Map.of("login_request_id", id, "state", state, "code", code); }
    }
    protected Result call(String method, String path, Object body, UUID user) {
        try (var client = HttpClient.newHttpClient()) {
            ...
            return new Result(response.statusCode(), response.body().isEmpty() ? null : mapper.readTree(response.body()), response.headers());
    ...
    protected Result callback(Pending pending) { return call("POST", "/auth/oauth/google/callback", pending.body(), null); }
    protected Result login(String subject) { var result = callback(pending(subject)); assertThat(result.status()).isEqualTo(200); return result; }
    protected Result refresh(String token) { return call("POST", "/auth/tokens/refresh", Map.of("refresh_token", token), null); }
    protected String rt(Result result) { return result.body().get("refresh_token").asString(); }
    protected void error(Result result, int status, String code, String nextAction) {
        ...
        assertThat(result.body().has("access_token")).isFalse(); assertThat(result.body().has("refresh_token")).isFalse();
    }
}
