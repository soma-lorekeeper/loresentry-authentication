...
class JwtTokensTest {
    ...
    private final JwtKeys keys = JwtKeys.load(TestKeys.PRIVATE, TestKeys.PUBLIC_FILE.toString(), TestKeys.KID);
    private final RsaJwtTokens tokens = new RsaJwtTokens(keys, Clock.fixed(NOW, ZoneOffset.UTC));

    @Test
    void issuedPairHasIndependentIdsCorrectLifetimesAndAnIndependentRsaSignature() throws Exception {
        ...
        assertThatThrownBy(() -> tokens.verifyRefresh(pair.accessToken(), false)).isInstanceOf(PortFailure.class);
    }
    ...
    void rejectsMissingWrongAndMalformedRequiredClaims() throws Exception {
        for (var name : List.of("sub", "iss", "aud", "iat", "exp", "jti", "token_type")) {
            var claims = validClaims(); claims.remove(name); reject(sign(claims, TestKeys.KID, JWSAlgorithm.RS256));
        }
        for (var entry : Map.<String, Object>of("sub", "1-1-1-1-1", "iss", "other", "aud", List.of("loresentry-api"), "jti", "invalid", "token_type", "access").entrySet()) {
            var claims = validClaims(); claims.put(entry.getKey(), entry.getValue()); reject(sign(claims, TestKeys.KID, JWSAlgorithm.RS256));
        }
        ...
        reject("malformed");
        var claims = validClaims(); claims.put("aud", List.of("other", "loresentry-auth"));
        assertThat(tokens.verifyRefresh(sign(claims, TestKeys.KID, JWSAlgorithm.RS256), false)).isNotNull();
    }
    ...
    void validatesTimeBoundariesAndAllowsOnlyExpirationToBeIgnoredForRevocation() throws Exception {
        ...
        assertThat(tokens.verifyRefresh(sign(claims, TestKeys.KID, JWSAlgorithm.RS256), false)).isNotNull();
        claims.put("iat", NOW.plusSeconds(31).getEpochSecond()); reject(sign(claims, TestKeys.KID, JWSAlgorithm.RS256));
        ...
        var expired = sign(claims, TestKeys.KID, JWSAlgorithm.RS256); reject(expired);
        ...
        assertThatThrownBy(() -> tokens.verifyRefresh(reversed, true)).isInstanceOf(PortFailure.class);
    }
    ...
    void replacedKeyRejectsPreviouslyIssuedTokensEvenWithReusedKid() throws Exception {
        ...
        var replacement = JwtKeys.load(Base64.getEncoder().encodeToString(pair.getPrivate().getEncoded()), TestKeys.publicFile(pair).toString(), UUID.randomUUID().toString());
        ...
        var sameKid = new RsaJwtTokens(new JwtKeys(replacement.privateKey(), replacement.publicKey(), TestKeys.KID), Clock.fixed(NOW, ZoneOffset.UTC));
        assertThatThrownBy(() -> sameKid.verifyRefresh(old, true)).isInstanceOf(PortFailure.class);
    }

    private Map<String, Object> validClaims() {
        return new HashMap<>(Map.of("sub", UUID.randomUUID().toString(), "iss", "loresentry-auth", "aud", List.of("loresentry-auth"),
                "iat", NOW.getEpochSecond(), "exp", NOW.plusSeconds(60).getEpochSecond(), "jti", UUID.randomUUID().toString(), "token_type", "refresh"));
    }
    private String sign(Map<String, Object> claims, String kid, JWSAlgorithm algorithm) throws Exception {
        var jwt = new SignedJWT(new JWSHeader.Builder(algorithm).keyID(kid).build(), JWTClaimsSet.parse(claims));
        jwt.sign(new RSASSASigner(keys.privateKey()));
        return jwt.serialize();
    }
    private void reject(String token) {
        assertThatThrownBy(() -> tokens.verifyRefresh(token, false)).isInstanceOfSatisfying(PortFailure.class,
                failure -> assertThat(failure.kind()).isEqualTo(PortFailure.Kind.INVALID_TOKEN));
    }
}
