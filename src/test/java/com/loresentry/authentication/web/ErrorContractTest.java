...
class ErrorContractTest {
    ...
    static class Fixture {
        @PostMapping("/auth/oauth/google/callback")
        void callback(@RequestBody Map<String, Object> body) {
            throw new AuthFailure(
                    AuthFailure.Reason.LOGIN_UNAVAILABLE,
                    AuthFailure.Consumption.valueOf((String) body.get("outcome")));
        }

        @GetMapping("/auth/test")
        void unexpected() {
            throw new IllegalArgumentException(
                    "secret-token oauth-code SELECT password client-secret provider-body");
        }
    }

    MockMvc mvc() {
        return MockMvcBuilders.standaloneSetup(new Fixture())
                .setControllerAdvice(new AuthExceptionHandler())
                .build();
    }

    @Test
    void everyDocumentedCodeHasExpectedStatusAndAction() {
        ...
        for (var reason : AuthFailure.Reason.values()) {
            ...
            assertThat(
                            response.getStatusCode().value()
                                    + " "
                                    + response.getBody().get("next_action"))
                    .isEqualTo(expected.get(reason));
            assertThat(response.getBody())
                    .containsOnlyKeys("code", "message", "next_action")
                    .containsEntry("code", reason.name());
            assertThat(response.getHeaders().getCacheControl()).isEqualTo("no-store");
        }
    }

    @Test
    void callbackPreservesAllThreeOutcomesAndMalformedJsonIsConfirmedNotConsumed()
            throws Exception {
        var mvc = mvc();
        for (var outcome : AuthFailure.Consumption.values()) {
            ...
            if (outcome == AuthFailure.Consumption.UNKNOWN)
                assertThat(body.get("login_request_consumed").isNull()).isTrue();
            else
                assertThat(body.get("login_request_consumed").booleanValue())
                        .isEqualTo(outcome == AuthFailure.Consumption.CONSUMED);
        }
        mvc.perform(
                        post("/auth/oauth/google/callback")
                                .contentType("application/json")
    ...
    @Test
    void unexpectedAndPreMvcFailuresUseSameSafeResponseAndLogs(CapturedOutput output)
            throws Exception {
        var response =
        ...
        var request = new MockHttpServletRequest("POST", "/auth/oauth/google/callback");
        var filtered = new MockHttpServletResponse();
        filter.doFilter(
                request,
                filtered,
                (req, res) -> {
        ...
        assertThat(parsed.get("code").asString()).isEqualTo("INTERNAL_ERROR");
        assertThat(parsed.get("login_request_consumed").isNull()).isTrue();
        for (String value :
                List.of(
                        "secret-token",
                        "oauth-code",
            ...
            assertThat(filtered.getContentAsString()).doesNotContain(value);
    ...
    private Map.Entry<AuthFailure.Reason, String> entry(String code, String value) {
        return Map.entry(AuthFailure.Reason.valueOf(code), value);
    }
}
